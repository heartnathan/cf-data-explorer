      ...
      <div v-else class="space-y-8">
        ...
        <section>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-if="d1Databases.length === 0" class="col-span-full py-8 text-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
              No D1 databases found.
            </div>
            <div 
              v-for="db in d1Databases" 
              :key="db.uuid"
              @click="navigateToItem('d1', db.uuid)"
            ...
            >
              <div>
                <h3 class="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-orange-500 transition-colors truncate" :title="db.name">{{ db.name }}</h3>
                <p class="text-xs text-slate-500 mt-1 font-mono truncate" :title="db.uuid">{{ db.uuid }}</p>
              </div>
              <div class="flex justify-between items-center text-xs text-slate-400">
                <span>{{ db.version }}</span>
                <span class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-orange-500">
                  Manage <el-icon><ArrowRight /></el-icon>
                </span>
              </div>
            </div>
          </div>
        ...
        <section>
          <div class="flex items-center gap-2 mb-4">
            <el-icon :size="24" color="#10b981"><DataLine /></el-icon>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-if="kvNamespaces.length === 0" class="col-span-full py-8 text-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
              No KV namespaces found.
            </div>
            <div 
              v-for="kv in kvNamespaces" 
              :key="kv.id"
              @click="navigateToItem('kv', kv.id)"
            ...
            >
              <div>
                <h3 class="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-emerald-500 transition-colors truncate" :title="kv.title">{{ kv.title }}</h3>
              ...
              <div class="flex justify-end items-center text-xs text-slate-400">
                <span class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-emerald-500">
                  Manage Keys <el-icon><ArrowRight /></el-icon>
                </span>
              </div>
            </div>
          </div>
        ...
        <section>
          <div class="flex items-center gap-2 mb-4">
            <el-icon :size="24" color="#3b82f6"><Files /></el-icon>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-if="r2Buckets.length === 0" class="col-span-full py-8 text-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
              No R2 buckets found.
            </div>
            <div 
              v-for="r2 in r2Buckets" 
              :key="r2.name"
              @click="navigateToItem('r2', r2.name)"
            ...
            >
              <div>
                <h3 class="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-blue-500 transition-colors truncate" :title="r2.name">{{ r2.name }}</h3>
              ...
              <div class="flex justify-between items-center text-xs text-slate-400">
                ...
                <span class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-blue-500">
                  Browse <el-icon><ArrowRight /></el-icon>
                </span>
              </div>
            </div>
          </div>
